...
locals {
  ...
  service_name                     = "file-transfer-api"
  service_name_secure              = "file-transfer-api-secure"
  container_port                   = "3000"                                  # default node port required here until prod docker container is built allowing port change via env var
  docker_repo                      = "file-transfer-api"
  ...
  healthcheck_path                 = "/file-transfer-api/health"            # healthcheck path for file-transfer-api
  healthcheck_matcher              = "200"
  kms_alias                        = "alias/${var.aws_profile}/environment-services-kms"
  vpc_name                         = local.service_secrets["vpc_name"]
  s3_config_bucket                 = data.vault_generic_secret.shared_s3.data["config_bucket_name"]
  app_environment_filename         = "file-transfer-api.env"
  app_environment_filename_secure  = "secure-file-transfer-api.env"
  ...
  additional_containers = [
    {
      ...
      dependsOn : [{
        containerName : "${local.service_name_secure}",
        condition : "START"
      }],
      essential : true
    }
