...
locals {
  ...
  vault_stack_path                 = "${var.protect_regime ? "secure-" : ""}applications/${var.aws_profile}/${var.environment}/${local.stack_name}-stack"
  eric_port                        = "10000"
  container_port                   = "8080"
  docker_repo                      = "file-transfer-service"
  lb_listener_rule_priority        = 10
  lb_listener_rule_priority_secure = 30
  ...
  kms_alias                        = var.protect_regime ? "alias/${var.protect_account}/euw2/environment-services-kms" : "alias/${var.aws_profile}/environment-services-kms"
