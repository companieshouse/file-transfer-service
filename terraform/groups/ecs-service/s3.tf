...
resource "aws_kms_grant" "file_transfer_encryption_key_grant" {
  ...
  grantee_principal = aws_iam_role.file_transfer_api_execution.arn
  operations        = ["Encrypt", "Decrypt", "GenerateDataKey"]
}

resource "aws_iam_role" "file_transfer_api_execution" {
  name               = "${var.stage}-file-transfer-api-ecs-execution"
  assume_role_policy = data.aws_iam_policy_document.file_transfer_api_trust.json
}
