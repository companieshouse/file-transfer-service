...
@TestPropertySource(properties = {
        "AWS_ACCESS_KEY_ID=valueFromTest",
        "AWS_SECRET_ACCESS_KEY=valueFromTest",
})
...
class FileTransferControllerIntegrationTest {
    ...
    static void overrideProperties(final DynamicPropertyRegistry registry) {
        ...
        registry.add("AWS_ACCESS_KEY_ID", () -> "noop");
        registry.add("AWS_SECRET_ACCESS_KEY", () -> "noop");
    }
    ...
    void shouldUpLoadAndDownloadTestFileToS3() throws Exception {
        try (FileInputStream is = new FileInputStream(ResourceUtils.getFile("classpath:large-file.pdf"))) {
            ...
            StopWatch watch = new StopWatch("shouldUpLoadAndDownloadTestFileToS3");
            ...
            MockHttpServletResponse uploadResult = mockMvc.perform(multipart("%s/".formatted(SERVICE_PATH)).file(file))
                    .andExpect(status().isOk())
                    .andReturn()
                    .getResponse();

            String fileId = uploadResult.getContentAsString();

            watch.stop();

            watch.start("Download");

            MockHttpServletResponse downloadResult = mockMvc.perform(get("/%s/download".formatted(fileId)))
                    .andExpect(status().isOk())
                    .andReturn()
                    .getResponse();
